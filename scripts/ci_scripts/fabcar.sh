...
for LANGUAGE in ${LANGUAGES}; do
    ...
    if [ -d ${LANGUAGE} ]; then
        ...
        copy_logs $? fabassets-${LANGUAGE}-enrollAdmin
        ${COMMAND} ${PREFIX}registerUser${SUFFIX}
        copy_logs $? fabassets-${LANGUAGE}-registerUser
        ${COMMAND} ${PREFIX}query${SUFFIX}
        copy_logs $? fabassets-${LANGUAGE}-query
        ${COMMAND} ${PREFIX}invoke${SUFFIX}
        copy_logs $? fabassets-${LANGUAGE}-invoke
        popd
    fi
    docker ps -aq | xargs docker rm -f
    docker rmi -f $(docker images -aq dev-*)
    echo -e "\033[32m finished fabassets test (${LANGUAGE})" "\033[0m"
done
