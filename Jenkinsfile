...
// Jenkinfile will get triggered on verify and merge jobs and run byfn, eyfn and fabassets
// tests.
            ...
            script {
              ...
              wrap([$class: 'AnsiColorBuildWrapper', 'colorMapName': 'xterm']) {
                try {
                  dir("$ROOTDIR/$BASE_DIR/scripts/ci_scripts") {
                    // Run fabassets tests
                    sh './ciScript.sh --fabassets_Tests'
                  }
                }
                catch (err) {
                  failure_stage = "fabassets_Tests"
                  currentBuild.result = 'FAILURE'
                  throw err
                }
              }
